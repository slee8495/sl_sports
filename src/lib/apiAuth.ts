...
// no Origin/Referer at all, so it goes through the x-api-key check instead.
const TRUSTED_HOSTS = [/^slsports\.vercel\.app$/, /-sl-studio\.vercel\.app$/, /^localhost:\d+$/];
...
export function isAuthorizedRequest(req: NextRequest): boolean {
  ...
  if (key && key === process.env.KEYBOARD_API_KEY) return true;
